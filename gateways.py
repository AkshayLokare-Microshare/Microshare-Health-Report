...
def color_output_in_excel(excel_path):
    ...
    for row in sheet.iter_rows():
        for cell in row:
            if cell.value == 'Gateway Dashboard is working':
                cell.fill = greenColor
            elif cell.value == 'Gateway Dashboard is NOT working':
                cell.fill = redColor
...
def gateways():
    ...
    with open(f'Health_Report({date}).txt', 'a') as file:
        try:
            ...
            gateway_dashboard = driver.find_element(By.CSS_SELECTOR, 'a[data-test-subj="dashboardListingTitleLink-Dashboard---Gateway-Status"]').click()
            time.sleep(5)

            panel_button = wait.until(EC.presence_of_element_located((By.XPATH, "//button[@aria-label='Panel options for Gateway Status Discovery2']")))
            panel_button.click()
            time.sleep(2)
            maximise_panel = driver.find_element(By.XPATH, "//span[contains(text(),'Maximize panel')]").click()
            ...
            file.write(f'\nLatest gateway component Date: {dashboard_date} | Dashboard Hour: {dashboard_hour} | Dashboard Min: {dashboard_minute} | Dashboard Sec: {dashboard_sec}\n')

            # Calculate minute and second differences
            hour_diff = abs(int(current_hour) - int(dashboard_hour))
            minute_diff = abs(int(current_minute) - int(dashboard_minute))
            second_diff = abs(int(current_sec) - int(dashboard_sec))

            if hour_diff <= 2 and minute_diff <= 30:
                file.write(f'Gateway Dashboard is working - {date}')
                output = 'Gateway Dashboard is working'
            else:
                file.write(f'Gateway Dashboard is NOT working - {date}')
                output = 'Gateway Dashboard is NOT working'
